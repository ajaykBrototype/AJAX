...
export const addToCartService = async (userId, variantId, quantity) => {
  ...
  if (existingItem) {
    ...
    cart.items.push({ productId: variant.productId, variant: variantId, quantity: Math.min(quantity, 5) });
  }
...
export const getCartService = async (userId) => {
  ...
  if (!cart) return { cart: null, subtotal: 0, totalDiscount: 0, totalPrice: 0 };
  ...
  return { cart, subtotal, totalDiscount, totalPrice };
};

export const updateCartQtyService = async (userId, itemId, delta) => {
  ...
  if (newQty > 5) return { success: false, message: "Max 5 allowed" };
  if (newQty > variant.stock) return { success: false, message: "Out of stock" };
  ...
  return { success: true, qty: newQty, subtotal, totalDiscount, totalPrice };
};
